...
\begin{equation}\label{fDef}\mbox{let $f = (\lambda pairs. join(\deno{P'}, pairs))$}\end{equation}
$$\mbox{then}$$
...
\end{equation}
$$\mbox{We can prove the forwards implication using the induction hypothesis $H(n, P') = (a, b)\in f^n \deno{Id_A} \Rightarrow \opRule{(a, b)}{(A, A)}{Exactly(n, P')}$}$$
...
& \Rightarrow \opRule{(a, b)}{(A, A)}{Exactly(0, P')}
\end{split}
\end{equation}
...
& \Rightarrow \opRule{(a, b)}{(A, A)}{Exactly(n+1, P')}\mbox{ by (Exactly n+1)}\\
\end{split}
\end{equation}
...
Todo:
\begin{equation}\label{exactlyReverse}
\begin{split}
	\opRule{(a, b)}{(A, A)}{Exactly(n, P')} & \Rightarrow ( n = 0\wedge \opRule{(a, b)}{(A, A)}{Id_A}) \vee (n = m + 1 \wedge \opRule{(a, b)}{}{Exactly(m, P')})
\end{split}
\end{equation}
