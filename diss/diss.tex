...
Do you motivate the interest in graph databases in this first few sentences? Might you open by describing the weakness/limitations of the SQL approach?

"For example, a program may be set up to read trade data from a database in a finance firm." - could this contain a little more information, hinting why this approach is limiting? I know you go on to describe but it good perhaps be clearer. Is it simply that the wrapper is a rigid interface to the underlying database? 

p.13 "(These" should this be capitalized? Moreover, it is probably better to break this long sentence up anyway.

Figure 2.1
I would quickly redraw this if you can't obtain it as a vector graphic (sometime you can extract a figure from the PDF?).
...
I might be tempted to use the phrase "reproduced from Angles et al[5]"

Looking at your Bibliography, I would try to ensure that full references are used rather than pointers to web-pages and ArXiv, i.e. if papers have been published at conferences or in journals reference these.

Even reference 12 looks short, e.g. who was the publisher?

p.25 - Is this list of tools better given in a table? (it doesn't have to have lines around the cells). At the moment some of the names overflow into the left margin.

Chapter 3. - Implementation

p.34 The line spacing is wrong from 3.2.2. onwards for some reason?
...
Figure 4.6 needs tidying, Y-label currently obscured. 

5.1 "work in areas" -> "work in the areas of" ?
"tests and tweaks" sounds too informal.

You use "finance firms" throughout, is there a better phrase? 
\end{comment}
