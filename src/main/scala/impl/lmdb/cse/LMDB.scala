...
object LMDB extends DBBackend {
  ...
  private def getInstance(address: DatabaseAddress, schema: SchemaDescription)(implicit ec: ExecutionContext): CSELMDBInstance = {
    val env = create()
    ...
    address match {
      case Empty =>
        ...
        new TemporaryCSELMDBInstance(env.open(dir.toFile), schema, dir)

      case DBDir(dir, _, _) =>
        val directory = dir.toFile
        if (!directory.exists()) {
          directory.mkdirs()
          new NewCSELMDBInstance(env.open(directory), schema)
        } else {
          new ExistingCSELMDBInstance(env.open(directory), schema)
        }
    }
  }
}
