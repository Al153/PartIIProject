...
trait FindAll { self: Methods =>
  ...
  /**
    ...
    */
  def findAll[A](t: FindSingle[A])(implicit sa: SchemaObject[A], sd: SchemaDescription): Operation[E, Vector[A]] =
  ...
  /**
    ...
    */
  def findDistinct[A](t: FindSingle[A])(implicit sa: SchemaObject[A], sd: SchemaDescription): Operation[E, Set[A]] =
    new ReadOperation({view: View => LMDBFutureE(for {
    // Check the view is valid
      _ <- instance.controlTables.availableViews.validateView(view)
      // to type erasure on the findSingle
