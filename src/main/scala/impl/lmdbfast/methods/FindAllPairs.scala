...
import core.utils._
import impl.lmdbfast.LMDBFutureE
import impl.lmdbfast.errors.{LMDBMissingRelation, LMDBMissingTable}
...
trait FindAllPairs { self: Methods =>
  ...
  def findAllPairs[A, B](
    ...
    // interpret the ADT
    res <- readVectorPairs[A, B](ut, commits, leftTable, rightTable)
  } yield res).asCFuture})
