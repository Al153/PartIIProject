...
trait PathFinding { self: ExecutorMethods =>
  ...
  def allShortestPathsImpl(start: Set[MemoryObject], searchStep: MemoryObject => MemoryEither[Set[RelatedPair]]):  MemoryEither[Set[MemoryPath]] =
    algorithms.PathFinding.allShortestPathsImpl(start, searchStep, EmptyFringeError).map(_.map(MemoryPath.apply))
  ...
  def singleShortestsPathImpl(start: Set[MemoryObject], end: UnsafeFindable, searchStep: MemoryObject => MemoryEither[Set[RelatedPair]], tree: MemoryTree): MemoryEither[Option[MemoryPath]] =
    for {
      ...
      res <- e.fold(MemoryEither[Option[MemoryPath]](None)){
        e => algorithms.PathFinding.singleShortestsPathImpl[MemoryError, MemoryObject](start, e, searchStep, EmptyFringeError).map(_.map(MemoryPath.apply))
      }
    } yield res
}